--1. How do I get full table with all orders? 

SELECT date, session.ga_session_id, continent, country, device, browser, mobile_model_name, operating_system, language, params.name AS traffic_source, channel, id AS account_id,
CASE WHEN  is_verified = 1 THEN "verified" ELSE "not verified" END AS is_verified,
CASE WHEN is_unsubscribed = 1 THEN "unsubscribed" ELSE "subscribed" END AS is_subscribed,
category, product.name, price, short_description
...
LEFT JOIN `DA.account_session` AS acc_session
ON session.ga_session_id = acc_session.ga_session_id
LEFT JOIN `DA.account` AS account 
ON acc_session.account_id = account.id
JOIN `DA.product` AS product
ON orders.item_id = product.item_id

--2. What is the average, minimum, and maximum price of an item for each category?
...
--3. What are the most profitable categories and their % of total_revenue? 
...
--4. What is the most purchased item and how many times all items were purchased? 
...
--5. How many emails were opened and visited by each device? 
...
--6. Revenue (total, desktop, mobile) and number of accounts (total and verified) by continent
...
--7. What is monthly revenue and it’s % of total revenue? 
